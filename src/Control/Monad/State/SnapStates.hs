...
The @'SnapStatesT'@ transformer is simply the @'Resumption'@
...
over the monad transformer @'StateT' s m@.
However, it is useful to understand what this monad looks like
in this instance.

First, we look at the underlying monad, @'SnapStates'@. A value of
@'SnapStates' s a@ is of the following shape (ignoring @newtype@
 constructors):

@
\\s -> (Free (Free (Pure a, s3), s2), s1)
@

The state @s1@ is a part of the outer @'State'@ monad, while @s2@
and @s3@ are parts of the inner
@'Control.Monad.Free.Free' 'Control.Monad.Writer.Writer'@
structure. However, it is the innermost state (@s3@ in the example)
that is somewhat special. Consider it the /current/ state. It
serves as the initial state for the bound continuation, but it does
not survive the bind operation:

@
\\s -> (Free (Free (Pure a, s3), s2), s1)
\>\>=
\\a -> \\s -> (Free (Pure b, s5), s4)
\ 
=
\ 
